...
문제점: (정책 위반 사항. 가능하면 관련 정책 섹션명 또는 조항 번호를 괄호 안에 인용. 예: [Google Play 개발자 정책 §4.3 가챠 확률표기])
권장사항: (실행 가능한 해결 방법)

[APPLE_REPORT]
판정: PASS/WARNING/REJECT
문제점: (정책 위반 사항. 가능하면 관련 정책 섹션명 인용. 예: [App Store 심사 지침 §3.1.1 인앱 구매])
권장사항: (실행 가능한 해결 방법)

[TOSS_REPORT]
판정: PASS/WARNING/REJECT
문제점: (정책 위반 사항. 가능하면 관련 조항 인용)
권장사항: (실행 가능한 해결 방법)

중요:
- 모든 분석 내용은 반드시 한글로 작성해주세요.
- 이 분석은 공개 정책 기반 추정이며 실 심사 결과·법률 자문이 아닙니다.
"""
